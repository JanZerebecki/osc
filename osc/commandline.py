...
#                    sys.exit(1)
    ...
    def do_service(self, subcmd, opts, *args):
        ...
        command = args.pop(0)
        if command not in ("runall", "ra", "run", "localrun", "precommit", "manualrun", "disabledrun", "remoterun", "lr", "dr", "mr", "r", "rr", "merge", "wait"):
        ...
        if command in ('runall', 'ra', 'run', 'localrun', 'precommit', 'manualrun', 'disabledrun', 'lr', 'mr', 'dr', 'r'):
            p = Package(".")
            if command  in ("localrun", "lr"):
                mode = "local"
